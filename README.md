...
- 新增信号路由时，自动把实际参与 Mapping 的 Rx、Tx `Signal Access` 设置为 `ACCESS_NEEDED_BY_SWC_OR_COM`。
- 根据目标 CAN 通道和基准 ARXML 的真实引用，自动维护直接报文目标腿与既有 PduR RoutingGroup 的成员引用。
- 同一目标通道存在应用组和专项组时，会结合真实成员通道与应用组名称交叉确认；专项组保持原样，不会接收普通新增路由。
- 应用路由组中存在完整的 Com、CanTp 或 DoIP 目标成员时，只将其排除在普通 CAN 通道统计之外，不会让组内正常的 CanIf 成员整体失效。
- 通过 CanIf、EcuC、PduR 的真实引用识别旧名称或人工名称的既有直接报文路由，避免仅因对象名不同重复创建。
- 新增目标报文时，如果完整既有源 Rx 的 CAN 类型与配置表默认值不同，会采用基准 ARXML 中的类型并保持源对象不变。
- 支持同一源端的一对多，只删除指定目标腿。
...
`PduRDestPdu → PduR BswModule → EcuC PDU → CanIfTxPdu → TxBuffer` 引用关系，结合“引用数据”中的目标
...
- “目标端已被本地发送占用”：只找到同名、同 CAN ID、同通道的本地自发 Tx，没有普通应用 PduR 目标腿作为复用证据。工具不会误判为既有网关路由，也不会重复创建第二套端点。
- “未解析到应用路由组/成员重复/悬空引用/删除后为空组”：工具会停止输出。DELETE 会反查目标腿实际所属组，并与目标通道的主应用组核对，避免误删异常归组或被多个 RoutingGroup 使用的 DestPdu。
- “路由组非主成员”：表示基线 ARXML 中已有成员落在该组的非主通道。工具会继续使用主通道映射，但把每个异常成员作为基线警告显示一次，不会把它错误归到某条 Excel 路由，也不会自动迁移成员。
- “非 CanIf 成员已从通道统计排除”：表示该成员的模块引用和 PDU 引用完整，但目标不是普通 CanIf 发送端。提示只归属基准 ARXML、同一问题只显示一次；无需删除 Excel 需求，工具会继续处理其他 CanIf 成员和无关通道。

新增信号路由时，工具会同时更新源端 Rx 和目标端 Tx 的 `ComSignalAccess`。如果上一版输出已经存在 Mapping、但该值仍为 `ACCESS_UNCLEAR`，使用同一配置重新生成也会补齐。DELETE 不会自动回退该值，因为工具无法证明信号是否仍被 SWC、COM 或其他路由使用。
